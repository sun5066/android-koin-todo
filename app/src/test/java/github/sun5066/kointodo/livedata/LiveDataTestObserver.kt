package github.sun5066.kointodo.livedata
...
class LiveDataTestObserver<T> : Observer<T> {
    ...
    fun assertValueSequence(sequence: List<T>): LiveDataTestObserver<T> {
        ...
        while (true) {
            ...
            if (actual != expected) throw AssertionError("actual: $actual, expected: $expected, index: $i")
            i++
        }

        if (actualNext) throw AssertionError("More values received than expected ($i)")
        if (expectedNext) throw AssertionError("Fewer values received than expected ($i)")
